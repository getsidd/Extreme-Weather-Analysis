...
if __name__ == "__main__":
	...
	csv_data = sc.textFile("/gpfs/courses/cse603/students/suryasid/Data_Four/*.csv",use_unicode=False)\
	...
	csv_data_as_kv_snwd = csv_data.map(lambda x: (x[6]+x[7][0:6],float(x[2])))\
							 .map(removeNull)\
							 .mapValues(lambda x:(x,1))\
							 .reduceByKey(lambda x,y:(x[0]+y[0],x[1]+y[1]))
	csv_data_as_snwd_map = csv_data_as_kv_snwd.map(lambda (key,xy):(key,xy[0]/xy[1])).collectAsMap()
	...
	csv_data.final = csv_data.map(fillMissingValues)\
							 ...
							 .map(lambda (x,y,z,w,u,v): gen_row([x,y,z,w,u,v]))
	data_parsed = csv_data.final.map(parsePoint)
	data_parsed.cache()
	model = SVMWithSGD.train(data_parsed, iterations=100)
	labelsAndPreds = data_parsed.map(lambda p: (p.label, model.predict(p.features)))
	trainErr = labelsAndPreds.filter(lambda (v, p): v != p).count() / float(data_parsed.count())
